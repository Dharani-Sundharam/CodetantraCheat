...
async function loadDashboard() {
    try {
        ...
        if (profileResponse && profileResponse.ok) {
            ...
            if (creditsBalanceEl) {
                creditsBalanceEl.textContent = profile.credits;
            }
            
            // Update referral code
            const referralCodeEl = document.getElementById('referralCode');
            if (referralCodeEl) {
                referralCodeEl.value = profile.referral_code;
            }
        }
...
async function loadUsageHistory() {
    try {
        const response = await apiRequest('/user/usage-history');
        if (response && response.ok) {
            const data = await response.json();
            const historyContainer = document.getElementById('usageHistory');
            
            if (historyContainer && data.usage_logs) {
                historyContainer.innerHTML = '';
                
                if (data.usage_logs.length === 0) {
                    historyContainer.innerHTML = '<p class="no-data">No usage history yet</p>';
                    return;
                }
                ...
                data.usage_logs.forEach(log => {
                    ...
                    row.innerHTML = `
                        ...
                        <td>${log.problem_type.replace('_', ' ')}</td>
                        <td><span class="status-badge ${statusClass}">${statusText}</span></td>
                        <td>-${log.credits_used} credits</td>
                    `;
    ...
    try {
        ...
        if (response && response.ok) {
            ...
            if (data.usage_logs) {
                ...
                const totalProblems = data.usage_logs.length;
                const creditsUsed = data.usage_logs.reduce((sum, log) => sum + log.credits_used, 0);
